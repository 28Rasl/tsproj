...
class TakeCommand extends Command_1.default {
    get cOptions() {
        return {
            prefix: '/',
            suppressArgs: true,
    ...
    async execute(message, args, { guild }) {
        ...
        const embed = {
            color: config.meta.defaultColor,
            description: `Со счета ${targetMember} снято **${amount.toLocaleString('ru-RU')}${Util.resolveEmoji(config.meta.emojis[type === 'gold' ? 'cy' : 'donateCy']).trim()}**`
        };
        message.channel
            .send({ embed })
